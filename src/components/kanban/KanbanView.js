...
function getFieldOptions(S, ent, field) {
  ...
  if (field === 'devId') return S.employees.filter(e => e.role === 'dev').map(e => ({ id: e.id, name: e.name, color: colorOf(e) }));
  if (field === 'agentId') return S.employees.filter(e => e.role === 'agent').map(e => ({ id: e.id, name: e.name, color: colorOf(e) }));
  return [];
}
